...
role doesn't itself require CAG review (CAG takes priority). Verifies
routing (Employee -> AAG -> HR -> AO, no HOD stage for this track), the
recommend/reject actions, matching the AAG assigned to the employee's
specific division, backend enforcement of the mandatory stage, and that the
normal HOD/CAG tracks are unaffected.
"""
import datetime
