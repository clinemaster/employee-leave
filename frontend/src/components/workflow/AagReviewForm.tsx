...
import { useAagReviewLeaveApplicationMutation, useAagRejectLeaveApplicationMutation } from "@/features/leave/leaveApi";
import { extractErrorMessage } from "@/lib/api/errors";
import {
  aagRejectSchema, aagReviewSchema, type AagRejectFormValues, type AagReviewFormValues,
...
export function AagReviewForm({ applicationId }: { applicationId: number }) {
  ...
  const [aagReject, { isLoading: isRejecting }] = useAagRejectLeaveApplicationMutation();
  ...
  } = useForm<AagReviewFormValues>({
    resolver: zodResolver(aagReviewSchema),
  ...
  } = useForm<AagRejectFormValues>({
    resolver: zodResolver(aagRejectSchema),
    defaultValues: { comments: "" },
  });

  async function onRecommend(values: AagReviewFormValues) {
    setActionError(null);
    try {
      await aagReview({
        id: applicationId,
      ...
      router.push("/aag/applications");
    } catch (err) {
      setActionError(extractErrorMessage(err, "Failed to submit recommendation. Please try again."));
    }
  }

  async function onReject(values: AagRejectFormValues) {
    setActionError(null);
    try {
      await aagReject({ id: applicationId, comments: values.comments }).unwrap();
      router.push("/aag/applications");
    } catch (err) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>AAG Review</CardTitle>
      </CardHeader>
      {actionError ? (
