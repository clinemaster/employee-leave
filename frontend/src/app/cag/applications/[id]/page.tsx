...
import { CagReviewForm } from "@/components/workflow/CagReviewForm";
import { useGetLeaveApplicationQuery } from "@/features/leave/leaveApi";

export default function CagApplicationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
    ...
    <AppShell>
      ...
      {isLoading || !application ? (
        ...
        <div className="space-y-4">
          ...
          <CagReviewForm applicationId={applicationId} />
        </div>
      )}
    </AppShell>
  );
