...
import { SectionA } from "@/components/leave/SectionReadOnly";
import { AagReviewForm } from "@/components/workflow/AagReviewForm";
import { useGetLeaveApplicationQuery } from "@/features/leave/leaveApi";

export default function AagApplicationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
    ...
    <AppShell>
      ...
      {isLoading || !application ? (
        ...
        <div className="space-y-4">
          ...
          <AagReviewForm applicationId={applicationId} />
        </div>
      )}
    </AppShell>
  );
